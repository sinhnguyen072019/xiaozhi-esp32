...
void McpServer::AddCommonTools() {
    ...
    AddTool("self.music.play", "Play a local music/audio track by filename from Flash memory.",
            PropertyList(
    ...
    AddTool("self.music.stop", "Stop local music playback.", PropertyList(),
            [](const PropertyList& properties) -> ReturnValue {
                LocalMusicPlayer::GetInstance().Stop();
                return true;
            });

    AddTool("self.special_days.list",
            "List all special days and greetings configured on the device. Days with 'readonly': true are hardcoded into firmware and cannot be modified or removed.",
            PropertyList(), [](const PropertyList& properties) -> ReturnValue {
                cJSON* json_arr = cJSON_CreateArray();
                for (int i = 0; i < NUM_SPECIAL_DAYS; ++i) {
                    cJSON* item = cJSON_CreateObject();
                    cJSON_AddNumberToObject(item, "month", SPECIAL_DAYS[i].month + 1);  // 1-12 for human reading
                    cJSON_AddNumberToObject(item, "day", SPECIAL_DAYS[i].day);
                    cJSON_AddStringToObject(item, "greeting", SPECIAL_DAYS[i].greeting ? SPECIAL_DAYS[i].greeting : "");
                    cJSON_AddBoolToObject(item, "readonly", true);
                    cJSON_AddItemToArray(json_arr, item);
                }
                auto days = GetCustomSpecialDays();
                for (const auto& d : days) {
                    if (IsHardcodedSpecialDay(d.month, d.day)) continue;
                    cJSON* item = cJSON_CreateObject();
                    cJSON_AddNumberToObject(item, "month", d.month + 1);  // 1-12 for human reading
                    cJSON_AddNumberToObject(item, "day", d.day);
                    cJSON_AddStringToObject(item, "greeting", d.greeting.c_str());
                    cJSON_AddBoolToObject(item, "readonly", false);
                    cJSON_AddItemToArray(json_arr, item);
                }
                return json_arr;
            });
    ...
    AddTool("self.special_days.add",
            ...
            [](const PropertyList& properties) -> ReturnValue {
                ...
                auto greeting = properties["greeting"].value<std::string>();
                bool success = AddCustomSpecialDay(month - 1, day, greeting);
                return success;
            });
    ...
    AddTool("self.special_days.remove",
            ...
            [](const PropertyList& properties) -> ReturnValue {
                int month = properties["month"].value<int>();
                int day = properties["day"].value<int>();
                bool success = RemoveCustomSpecialDay(month - 1, day);
                return success;
            });

    AddTool("self.special_days.clear",
            "Clear all custom special days and reset to default greetings.",
            PropertyList(), [](const PropertyList& properties) -> ReturnValue {
                ClearCustomSpecialDays();
                return true;
            });
